...
The bot runs as a Docker service alongside the backend on the VM.

### Prerequisites

- `.env.docker.secret` configured with bot credentials (see below)
- Backend already running via `docker compose up`
...
LMS_API_KEY=<same-key-as-backend>
